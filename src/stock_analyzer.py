...
class StockAnalyzer:
    ...
    # 上昇理由のキーワード
    CATALYST_KEYWORDS = {
        '大株主': ['大株主', 'TOB', '筆頭株主', '持株比率', '株主異動', '大量保有', '変更報告', '保有報告', 'ファンド', '取得', '保有', '共同保有'],
        '契約・提携': ['契約', '提携', '受注', '獲得', 'M&A', '買収', '子会社化', '経営統合'],
        '新製品・技術': ['新製品', '新技術', '開発', '実用化', '量産', '投入', '発売', '特許'],
        '株主還元': ['配当', '増配', '自社株買い', '株主還元', '特別配当', '分割'],
        '政策・規制': ['規制緩和', '政策', '補助金', '認可', '承認', '許可', '法改正'],
    ...
    def _consolidate_reasons(
        ...
        for news in news_list[:3]:  # 最新3件
            ...
            if category_match:
                ...
                # 重要な情報のみ追加
                if any(kw in content for kw in ['ストップ高', 'S高', '大株主', '大量保有', 'ファンド', '決算', '上方修正', '受注', '契約', '買収', 'TOB', '配当', '自社株買い']):
                    key_info.append(content)

        # カタリストから主要な理由を特定（IR・大株主を最優先、決算は最後）
        main_catalyst = ""
        if '大株主' in catalysts:
            main_catalyst = "大株主の異動・大量保有報告"
        elif '契約・提携' in catalysts:
            main_catalyst = "大型契約の獲得"
        elif '新製品・技術' in catalysts:
            main_catalyst = "新製品・新技術の発表"
        elif '株主還元' in catalysts:
            main_catalyst = "株主還元の強化"
        elif '政策・規制' in catalysts:
            main_catalyst = "政策・規制関連"
        elif '決算' in catalysts:
